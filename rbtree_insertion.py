...
class RBTree():
    ...
    def LR ( self , x ) :
        print(x.val)
        y = x.right
        print(y.val,x.right.val,y.left.val)                                      # Y = Right child of x
        x.right = y.left                                 # Change right child of x to left child of y
        if y.left != self.NULL :
            print(y.left.parent.val)
            y.left.parent = x
        print(y.parent.val,x.parent.val)
        y.parent = x.parent                              # Change parent of y as parent of x
        if x.parent == None :                            # If parent of x == None ie. root node
            self.root = y                                # Set y as root
        elif x == x.parent.left :
            print(x.parent.left.val,x.parent.val)
            x.parent.left = y
        else :
            print(x.parent.val)
            x.parent.right = y
        print(y.left.val,x.val,x.parent.val)
        y.left = x
        x.parent = y
    ...
    def RR ( self , x ) :
        print(x.val,x.left.val)
        y = x.left                                       # Y = Left child of x
        print(y.val,x.left.val,y.right.val)
...
if __name__ == "__main__":
    bst = RBTree()
    node=int(input("Enter the Node for Red-Black Tree :"))
    while node!=-1:
            bst.insertNode(node)
            node=int(input("Enter the Node for Red-Black Tree :"))
    bst.print_tree()
